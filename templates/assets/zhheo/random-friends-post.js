"use strict";
var fdata = {apiurl: "https://moments.zhheo.com/", defaultFish: 100, hungryFish: 100};
if ("undefined" != typeof fdataUser) for (var key in fdataUser) fdataUser[key] && (fdata[key] = fdataUser[key]);
var randomPostTimes = 0, randomPostWorking = !1,
...
function fetchRandomPost() {
    ...
    console.log(fdata.apiurl), 0 == randomPostWorking && (randomPostWorking = !0, s = randomPostTips[Math.floor(Math.random() * randomPostTips.length)], o = "", o = 1e4 < randomPostTimes ? "愿者上钩" : 1e3 < randomPostTimes ? "俯览天下" : 1e3 < randomPostTimes ? "超越神了" : 100 < randomPostTimes ? "绝世渔夫" : 75 < randomPostTimes ? "钓鱼王者" : 50 < randomPostTimes ? "钓鱼宗师" : 20 < randomPostTimes ? "钓鱼专家" : 5 < randomPostTimes ? "钓鱼高手" : "钓鱼新手", document.getElementById("random-post").innerHTML = 5 <= randomPostTimes ? "钓鱼中... （Lv." + randomPostTimes + " 当前称号：" + o + "）" : "钓鱼中...", r = randomNum(1e3, 3e3), 0 == randomPostTimes && (r = 0), $(".random-post-start").css("opacity", "0.2"), randomPostWorking = (randomPostClick * fdata.hungryFish + fdata.defaultFish < randomPostTimes && 0 == Math.round(Math.random()) ? document.getElementById("random-post").innerHTML = "因为只钓鱼不吃鱼，过分饥饿导致本次钓鱼失败...(点击任意一篇钓鱼获得的文章即可恢复）" : (t = fdata.apiurl + "randompost", fetch(t).then(function (o) {
        ...
        var t = o.title, a = o.link, n = o.author;
        document.querySelector("#random-post") && window.setTimeout(function () {
            document.getElementById("random-post").innerHTML = s + "来自友链 <b>" + n + '</b> 的文章：<a class="random-friends-post" onclick="randomClickLink()" target="_blank" href="' + a + '" rel="external nofollow">' + t + "</a>", randomPostTimes += 1, localStorage.setItem("randomPostTimes", randomPostTimes), $(".random-post-start").css("opacity", "1")
        }, r)
    })), !1))
}

function initRandomPost() {
    localStorage.randomPostTimes && (randomPostTimes = parseInt(localStorage.randomPostTimes), randomPostClick = parseInt(localStorage.randomPostClick), $(".random-post-start").css("transition-duration", "0.3s"), $(".random-post-start").css("transform", "rotate(" + 360 * randomPostTimes + "deg)")), fetchRandomPost()
}
